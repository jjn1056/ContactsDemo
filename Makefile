...
update_sqitch_conf:
	...
	@echo '[target "main"]' >> sqitch.conf
	@echo '    uri = db:pg:$(DB_NAME)?user=$(DB_USER)&password=$(DB_PASSWORD)&host=$(DB_HOST)&port=$(DB_PORT)' >> sqitch.conf
...
# Start, Stop and Restart the docker container
...
app-shell:
	@docker-compose exec catalyst_app bash

app-update:
	@docker-compose exec catalyst_app make update

app-hup:
	@docker-compose exec catalyst_app make hup

app-restart: app-update app-hup

db-psql:
	@docker-compose exec contactsdb psql -U contact_dbuser contacts

db-shell:
	@docker-compose exec contactsdb bash
